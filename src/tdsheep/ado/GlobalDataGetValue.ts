...
export class GlobalDataGetValue {
  ...
  static getLogMsgTemp(_msgObj, _global_config) {
    ...
    if (!Tools.isNullAndUndefined(_global_config)) {
      ...
      _fname = Boolean(_msgObj["friend_name"])
        ? _msgObj["friend_name"]
        : GlobalDataGetValue.getDataProfileTxt(GlobalData.$_user_default_name);
      ...
      if (_global_config.hasOwnProperty(_msgObj["msg_type"])) {
        ...
        for (i in _data) {
          ...
          if (_let == "user_name" && _data[i] == null) {
            _strRepalceTo = this.GlobalDataGetValue.getDataProfileTxt(
              GlobalData.$_user_default_name
            );
          } else if (_let == "p_str") {
      ...
      if (_isIndexOf) {
        ...
        if (GlobalData.$_global_dataProfile_txt != null) {
          if (GlobalData.$_global_dataProfile_txt.getElementsByTagName(_xmlId)[0] != undefined) {
            _str = GlobalData.$_global_dataProfile_txt
              .getElementsByTagName(_xmlId)[0]
              .getAttribute("s")
              .toString();
          } else {
            _str = _cnStr;
  ...
  static getToolsData_obj(_type, _id) {
    ...
    if (GlobalData.$_global_properties[_type]) {
      if (GlobalData.$_global_properties[_type][_id]) {
        _obj = GlobalDataGetValue.setObjecInConfigStr_format(
          GlobalData.$_global_properties[_type][_id]
        );
      }
    }
    return _obj;
  ...
  static checkGlobalSlave(_sysTime) {
    ...
    for (i in GlobalData.$_global_slave_myData) {
      _m =
        (_sysTime -
          new Date(GlobalData.$_global_slave_myData[i][SlaveBaseData.CATCH_TIME]).getTime() *
            0.001) /
        60;
  ...
  static checkSlaveIsWorking(_uid) {
    ...
    if (!Tools.isNullAndUndefined(GlobalData.$_global_slave_myData[_uid])) {
      _workTime =
        new Date(GlobalData.$_global_slave_myData[_uid][SlaveBaseData.WORK_TIME]).getTime() * 0.001;
  ...
  static formatGlobalMySlave(_mySlaveData) {
    ...
    for (i in _mySlaveData) {
      ...
      _slaveData[SlaveBaseData.ID_KEY] =
        _slaveData[SlaveBaseData.ID] + "_" + _slaveData[SlaveBaseData.TIME_KEY];
  ...
  static getCurrUserMapData() {
    return GlobalData.$_GAME_uid == GlobalData.$_GAME_fid
      ? GlobalData.$_userLevelData
      : GlobalData.$_friendLevelData;
  }
